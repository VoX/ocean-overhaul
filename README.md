...
All 73 recipes the mod adds, generated straight from the recipe JSON (`scripts/gen-recipe-docs.py`). Shaped patterns are shown row-by-row (`.` = empty slot) with a key legend; “Any Coral” / “Any Coral Block” means any of the five vanilla coral colors works.
...
Every crafting-table recipe the mod adds, rendered as the grid + result (blocks shown as 3D, slabs as half-blocks). See **Recipes** above for smelting & stonecutter.
