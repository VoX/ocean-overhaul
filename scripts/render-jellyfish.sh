...
# Derive MOD_VERSION from gradle.properties so this never goes stale on a version bump.
MOD_VERSION="$(sed -n 's/^[[:space:]]*mod_version[[:space:]]*=[[:space:]]*//p' "$REPO/gradle.properties" | tr -d '[:space:]')"
...
note "building mod"
( cd "$REPO" && ./gradlew build --no-daemon ) >/tmp/oo-render-jelly-build.log 2>&1 \
    || { tail -25 /tmp/oo-render-jelly-build.log; die "gradle build failed"; }
...
"$JAVA" -jar "$INSTALLER" server -mcversion "$MC_VERSION" -loader "$FABRIC_LOADER" \
    -downloadMinecraft -dir "$SCRATCH" >/tmp/oo-render-jelly-install.log 2>&1 \
    || { tail -20 /tmp/oo-render-jelly-install.log; die "fabric server install failed"; }
...
sleep 1
send "summon oceanstarter:jellyfish 7 100 7 {NoAI:1b,Silent:1b,PersistenceRequired:1b}"
sleep 4
grep -q 'Summoned new Jellyfish' "$LOG" && note "jelly summoned" || note "WARN: jelly summon line not seen"

# keep the boss pinned at the framing point each second (it has NoAI but belt+braces)
send "tp @e[type=oceanstarter:jellyfish,limit=1] 7 100 7"
...
VANTAGE_X=5; VANTAGE_Y=100; VANTAGE_Z=7
...
(
    ...
    while kill -0 "$GRADLE_PID" 2>/dev/null; do
        ...
        printf '%s\r\n' "execute unless entity @e[type=oceanstarter:jellyfish] run summon oceanstarter:jellyfish 7 100 7 {NoAI:1b,Silent:1b,PersistenceRequired:1b}" >&3 2>/dev/null || break
        printf '%s\r\n' "tp @e[type=oceanstarter:jellyfish,limit=1] 7 100 7" >&3 2>/dev/null || break
        sleep 1
    done
) &
WRANGLER_PID=$!
