...
- **1.21.11 API notes (post-1.21.1 jump — the big ones, all verified via javap):**
  ...
  - **Emissive masks MUST have a TRANSPARENT background (1.21.5 gotcha that shipped a bug):** `RenderLayers.eyes(...)` no longer blends purely additively — it composites with the mask's alpha. So an emissive/glow mask with an OPAQUE-black background (the old "black everywhere except the glow" trick that worked additively pre-1.21.5) now paints the whole body black, leaving only the glow poking through. The `EyesFeatureRenderer` re-renders the entire model through the mask, so the mask must be transparent (alpha 0) everywhere except the glowing pixels — the vanilla `spider_eyes.png` convention. `abyssal_lurker_emissive.png` was fixed to transparent-bg; keep any future glow mask transparent-bg.
