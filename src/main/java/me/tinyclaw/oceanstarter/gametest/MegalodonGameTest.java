...
public class MegalodonGameTest implements FabricGameTest {
	...
	public void megalodonSpawnPredicateGatesOnWaterDepthAndRarity(TestContext context) {
		...
		context.assertFalse(
				Megalodon.canSpawn(OceanStarter.MEGALODON, world, SpawnReason.NATURAL, dryAbs,
						seededWithZeroRoll()),
		...
		boolean actualWithZeroRoll = Megalodon.canSpawn(
				OceanStarter.MEGALODON, world, SpawnReason.NATURAL, wetAbs, seededWithZeroRoll());
		context.assertTrue(
		...
		if (deep) {
			context.assertFalse(
					Megalodon.canSpawn(OceanStarter.MEGALODON, world, SpawnReason.NATURAL, wetAbs,
			...
			for (long seed = 0L; seed < 256L && !anyTrue; seed++) {
				anyTrue = Megalodon.canSpawn(OceanStarter.MEGALODON, world, SpawnReason.NATURAL, wetAbs,
