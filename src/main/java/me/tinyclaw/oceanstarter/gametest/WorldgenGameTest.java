 ...
 * deep-ocean biomes (see {@link me.tinyclaw.oceanstarter.OceanStarterWorldgen}): three
...
public class WorldgenGameTest implements FabricGameTest {
	 ...
	 * placed_feature JSON under {@code data/oceanstarter/worldgen/}.
	 */
	...
	public void featuresResolveFromRegistry(TestContext context) {
		...
		for (String name : TRENCH_FEATURES) {
			ConfiguredFeature<?, ?> cf = configuredFeatures.get(OceanStarter.id(name));
			context.assertTrue(
					cf != null,
					"configured_feature oceanstarter:" + name + " did not resolve from the registry");

			PlacedFeature pf = placedFeatures.get(OceanStarter.id(name));
			context.assertTrue(
					pf != null,
					"placed_feature oceanstarter:" + name + " did not resolve from the registry");
		}
	...
	public void planktonPatchActuallyPlaces(TestContext context) {
		...
		ConfiguredFeature<?, ?> cf =
				world.getRegistryManager().get(RegistryKeys.CONFIGURED_FEATURE)
						.get(OceanStarter.id("glowing_plankton_patch"));
		...
		for (int dx = 0; dx <= 8 && !placed; dx++) {
			for (int dz = 0; dz <= 8 && !placed; dz++) {
				...
				if (state.isOf(OceanStarter.GLOWING_PLANKTON_BLOCK)) {
					placed = true;
				}
			}
		}
	...
	public void newBlocksPlaceBreakDrop(TestContext context) {
		...
		placeAndExpect(context, OceanStarter.GLOWING_PLANKTON_BLOCK, 0);
		assertDropsSelf(context, world, OceanStarter.GLOWING_PLANKTON_BLOCK, 0);

		// --- Abyssal Vent: places + drops itself. ---
		placeAndExpect(context, OceanStarter.ABYSSAL_VENT, 2);
		assertDropsSelf(context, world, OceanStarter.ABYSSAL_VENT, 2);

		// --- Giant Clam: places, but drops the GUARANTEED abyssal_pearl (not itself). ---
		placeAndExpect(context, OceanStarter.GIANT_CLAM, 4);
		...
		boolean droppedPearl = clamDrops.stream().anyMatch(s -> s.isOf(OceanStarter.ABYSSAL_PEARL));
		context.assertTrue(
				droppedPearl,
