package me.tinyclaw.oceanstarter.block;
...
public class AquariumBlock extends BlockWithEntity {
	...
	private static EntityType<?> bucketCreatureType(ItemStack stack) {
		if (stack.isOf(OceanStarter.REEF_FISH_BUCKET)) {
			return OceanStarter.REEF_FISH;
		}
		if (stack.isOf(OceanStarter.JELLYFISH_BUCKET)) {
			return OceanStarter.JELLYFISH;
		}
		return null;
	}
	...
	private static ItemStack buildFilledBucket(EntityType<?> type, int variant) {
		if (type == OceanStarter.JELLYFISH) {
