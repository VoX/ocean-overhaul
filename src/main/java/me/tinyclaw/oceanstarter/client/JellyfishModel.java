...
/**
 * Cuboid model for the {@link me.tinyclaw.oceanstarter.entity.Jellyfish} — a domed bell
 * with four dangling tentacles on a 32x32 atlas.
 *
 ...
 * {@link JellyfishRenderState}. It uses the two-arg {@code EntityModel(root,
 * layerFactory)} super to render on the TRANSLUCENT layer (alpha-blended) instead of the
 * EntityModel default ({@code RenderLayer::getEntityCutoutNoCull}, which is alpha-TESTED
 * — it discards low-alpha pixels and draws the rest fully opaque). The jellyfish texture
 * bakes semi-transparent pixels for the see-through bell, so it needs real blending or it
 * renders as a solid blob. (ReefFish/Megalodon keep the cutout default — opaque textures.)</p>
 *
...
public class JellyfishModel extends EntityModel<JellyfishRenderState> {
	...
	public static final EntityModelLayer LAYER =
			new EntityModelLayer(OceanStarter.id("jellyfish"), "main");
	...
	public static TexturedModelData getTexturedModelData() {
		...
		ModelPartData bell = root.addChild("bell",
				...
				ModelTransform.origin(0.0F, BELL_BASE_Y, 0.0F));
		...
		bell.addChild("tentacle_0",
				...
				ModelTransform.origin(-2.0F, 1.0F, -2.0F));
	...
	public void setAngles(JellyfishRenderState state) {
		...
		this.bell.originY = BELL_BASE_Y + MathHelper.cos(animationProgress * 0.15F) * 0.5F;
