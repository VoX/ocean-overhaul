...
/**
 * Cuboid model for the {@link me.tinyclaw.oceanstarter.entity.AbyssalLurker} — a classic
 * deep-sea ANGLERFISH: a bulbous, front-heavy body, an enormous gaping mouth bristling
 * with needle teeth, a big pale eye, and a bioluminescent lure (the esca) dangling on a
 ...
 * origin below is mirrored by the texture painter (paint_abyssal_lurker.py). The whole
 * thing unwraps onto a 128x128 atlas.</p>
 *
 * <p><b>Built at ~2-block scale.</b> The cuboids are sized natively so the silhouette
 * fills an elder-guardian hitbox (~1.9975 blocks); the renderer keeps scale 1.0 (no
 * {@code scale()} override) so the visual stays coupled to the hitbox.</p>
 *
 * <p>1.21.5+ render pipeline: extends {@link EntityModel} on a
 * {@link LivingEntityRenderState}; the default (opaque) cutout layer. The bulb's glow is a
 * separate emissive feature ({@link AbyssalLurkerEyesFeature}).</p>
 *
 * <p>Model-space convention (matches the rest of this mod): more-negative Y is UP,
 * more-negative Z is FORWARD (toward the snout).</p>
 */
public class AbyssalLurkerModel extends EntityModel<LivingEntityRenderState> {
	...
	public static TexturedModelData getTexturedModelData() {
		...
		// Head / upper jaw — the wide snout / forehead. Its lower face is the roof of the
		// gaping mouth. ~24 wide x 12 tall x 16 long.
		ModelPartData head = body.addChild("head",
				ModelPartBuilder.create().uv(0, 51).cuboid(-12.0F, -12.0F, -16.0F, 24.0F, 12.0F, 16.0F),
				ModelTransform.origin(0.0F, -8.0F, -10.0F));

		// Lower jaw — a deep slab hinged at the back of the mouth and dropped wide open
		// (pitch ~0.7 rad). Its top face is the mouth floor. ~22 wide x 7 tall x 16 long.
		...
		// the glowing esca bulb at its tip. The bulb is a child of the stalk so it swings
		// with it. uv(94,26) stalk / uv(104,26) bulb.
		ModelPartData lureStalk = head.addChild("lure_stalk",
				ModelPartBuilder.create().uv(94, 26).cuboid(-1.0F, -18.0F, -1.0F, 2.0F, 16.0F, 2.0F),
				ModelTransform.origin(0.0F, -10.0F, -14.0F));
		...
		ModelPartData tail = body.addChild("tail",
				...
				ModelTransform.origin(0.0F, -6.0F, 10.0F));
		...
		tail.addChild("caudal_fin",
				...
				ModelTransform.origin(0.0F, -2.0F, 12.0F));

		// Dorsal fin — a ridge on top of the body.
		...
		body.addChild("right_pectoral_fin",
				...
				ModelTransform.origin(-13.0F, 4.0F, -2.0F));
	...
	public void setAngles(LivingEntityRenderState state) {
		...
		// extends up (-Y), so a POSITIVE pitch tips its tip toward -Z (forward).
