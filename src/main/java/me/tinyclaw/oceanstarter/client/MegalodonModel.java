...
/**
 * Cuboid model for the {@link me.tinyclaw.oceanstarter.entity.Megalodon} — a proper shark
 * silhouette with a big gaping, toothy mouth.
 *
 ...
 *
 * <p>1.21.5+ render pipeline: extends {@link EntityModel} parameterised on a
 * {@link LivingEntityRenderState} (animation inputs come from that per-frame snapshot,
 * not the live entity). The root part is handed to the {@code Model} super, which owns
 * rendering; {@link #setAngles} only poses the animated parts.</p>
 *
 * <p>Model-space convention (matches the rest of this mod): more-negative Y is
 * UP, more-negative Z is FORWARD (toward the snout).</p>
 */
public class MegalodonModel extends EntityModel<LivingEntityRenderState> {
	...
	public static TexturedModelData getTexturedModelData() {
		...
		body.addChild("head",
				...
				ModelTransform.origin(0.0F, 0.0F, -16.0F));
		...
		body.addChild("dorsal_fin",
				...
				ModelTransform.origin(0.0F, 0.0F, -2.0F));
		...
		body.addChild("right_pectoral_fin",
				...
				ModelTransform.origin(-4.0F, 1.0F, -8.0F));
