...
public class BlockGameTest implements FabricGameTest {
	...
	private static final List<Block> ROUND_TRIP_BLOCKS = List.of(
			...
			OceanOverhaul.SEA_GLASS,                  // glass-like (non-opaque) full block
			OceanOverhaul.POLISHED_PRISMARINE_BRICKS, // brick full block
			OceanOverhaul.PEARL_BLOCK,                // quartz-like full block
			OceanOverhaul.BARNACLE_BLOCK,             // natural decorative full block
			OceanOverhaul.PEARL_LANTERN,              // luminous (luminance 15) full block
			OceanOverhaul.PRISMARINE_CRYSTAL_BLOCK,   // luminous full block
			OceanOverhaul.CHISELED_PRISMARINE_TILES,  // chiseled full block
			OceanOverhaul.PEARL_BLOCK_STAIRS,         // StairsBlock shape (has facing/half state)
			OceanOverhaul.POLISHED_PRISMARINE_BRICKS_SLAB, // SlabBlock shape (has type state)
			OceanOverhaul.DRIFTWOOD_PLANK_WALL,       // WallBlock shape (has connection state)
			OceanOverhaul.AQUARIUM);                  // BlockWithEntity (places + creates its BlockEntity)
	...
	public void saltBlockPlacesAsItself(TestContext context) {
		...
		context.setBlockState(at, OceanOverhaul.SALT_BLOCK);
		context.expectBlock(OceanOverhaul.SALT_BLOCK, at);
		context.complete();
	}
	...
	public void blocksDropThemselvesWhenMined(TestContext context) {
		...
		assertDropsSelf(context, world, OceanOverhaul.PEARL_BLOCK);
		assertDropsSelf(context, world, OceanOverhaul.SEA_GLASS);
		assertDropsSelf(context, world, OceanOverhaul.ABYSSAL_CORAL_BLOCK);
