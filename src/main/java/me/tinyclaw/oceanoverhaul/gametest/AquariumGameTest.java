...
public class AquariumGameTest implements FabricGameTest {
	...
	public void reefFishBucketCatchesViaTryBucket(TestContext context) {
		...
		context.assertTrue(
				caught.isOf(OceanOverhaul.REEF_FISH_BUCKET),
	...
	public void jellyfishBucketPreservesVariant(TestContext context) {
		...
		context.assertTrue(caught.isOf(OceanOverhaul.JELLYFISH_BUCKET),
	...
	public void aquariumBlockEntityNbtRoundTrips(TestContext context) {
		...
		be.setStored(OceanOverhaul.JELLYFISH, 4);
		NbtCompound nbt = be.createNbt(context.getWorld().getRegistryManager());
		context.assertTrue(
				"oceanoverhaul:jellyfish".equals(nbt.getString("StoredEntity")),
				"StoredEntity expected oceanoverhaul:jellyfish but was '" + nbt.getString("StoredEntity") + "'");
		context.assertTrue(
		...
		context.setBlockState(emptyPos, OceanOverhaul.AQUARIUM);
	...
	public void aquariumStoreViaBucket(TestContext context) {
		...
		context.assertTrue(
				be.storedType() == OceanOverhaul.JELLYFISH,
	...
	public void aquariumRetrieveViaEmptyBucket(TestContext context) {
		...
		AquariumBlockEntity be = context.getBlockEntity(pos);
		context.assertTrue(be != null, "Aquarium BlockEntity was null");
		be.setStored(OceanOverhaul.REEF_FISH, 0);
		...
		context.assertTrue(
				player.getStackInHand(Hand.MAIN_HAND).isOf(OceanOverhaul.REEF_FISH_BUCKET),
	...
	public void aquariumPlaceAndBreakLifecycle(TestContext context) {
		BlockPos pos = new BlockPos(1, 2, 1);
		context.setBlockState(pos, OceanOverhaul.AQUARIUM);
		context.expectBlock(OceanOverhaul.AQUARIUM, pos);
	 ...
	 * {@link me.tinyclaw.oceanoverhaul.block.AquariumBlock#onStateReplaced}, which the existing
	...
	public void aquariumScattersStoredCreatureBucketOnBreak(TestContext context) {
		...
		context.setBlockState(stockedRel, OceanOverhaul.AQUARIUM);
		AquariumBlockEntity be = context.getBlockEntity(stockedRel);
		context.assertTrue(be != null, "Aquarium BlockEntity was null after placement");
		be.setStored(OceanOverhaul.JELLYFISH, 3);
		...
		long scattered = bucketItemsNear(context, stockedAbs, OceanOverhaul.JELLYFISH_BUCKET, 3);
		context.assertTrue(
				scattered >= 1,
		...
		context.setBlockState(emptyRel, OceanOverhaul.AQUARIUM);
		...
		long fromEmpty = bucketItemsNear(context, emptyAbs, OceanOverhaul.JELLYFISH_BUCKET, null);
		context.assertTrue(
				fromEmpty == 0,
