 ...
 *
 * <p>It reuses the EXISTING entity models ({@link ReefFishModel} / {@link JellyfishModel} /
 * {@link SeahorseModel}) and their textures — no real entity is spawned client-side (no fake
 * tick/AI/lifecycle), so the only new render code is matrix transforms + a {@code model.render(...)}
 * call. The model layers are already registered for the live mobs in {@link OceanOverhaulClient},
 * so {@code ctx.getLayerModelPart} resolves without a new {@code EntityModelLayerRegistry} entry.</p>
 *
 ...
 * uses — with a {@code null} entity argument, which is VERIFIED null-safe: all three of
 * {@link ReefFishModel#setAngles}, {@link JellyfishModel#setAngles} and
 * {@link SeahorseModel#setAngles} read only {@code animationProgress}/{@code limbDistance}, never
 * the entity arg. Keep them null-safe.</p>
 */
public class AquariumBlockEntityRenderer implements BlockEntityRenderer<AquariumBlockEntity> {
	...
	/** Seahorse per-variant textures (same paths + order {@link SeahorseRenderer} uses). */
	private static final Identifier[] SEAHORSE_TEXTURES = {
	...
	public AquariumBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
		...
		this.seahorseModel = new SeahorseModel(ctx.getLayerModelPart(SeahorseModel.LAYER));
	}
	...
	public void render(AquariumBlockEntity be, float tickDelta, MatrixStack matrices,
		...
		// Three-way species selection keyed on storedType: model / texture / yOffset / scale /
		// light per species. The jelly renders full-bright (a self-lit neon creature, matching
		// JellyfishRenderer); the reef fish and seahorse use the block's own light (not
		// bioluminescent).
		...
		float scale;
		int renderLight;
		if (be.storedType() == OceanOverhaul.JELLYFISH) {
			model = jellyfishModel;
			texture = JELLYFISH_TEXTURES[
					MathHelper.clamp(be.storedVariant(), 0, JELLYFISH_TEXTURES.length - 1)];
			yOffset = -0.15F;
			scale = 0.45F;
			renderLight = LightmapTextureManager.MAX_LIGHT_COORDINATE;
		} else if (be.storedType() == OceanOverhaul.SEAHORSE) {
			model = seahorseModel;
			texture = SEAHORSE_TEXTURES[
					MathHelper.clamp(be.storedVariant(), 0, SEAHORSE_TEXTURES.length - 1)];
			yOffset = -0.05F;
			scale = 0.4F;
			renderLight = light;
		} else {
			model = reefFishModel;
			texture = REEF_FISH_TEXTURE;
			yOffset = -0.1F;
			scale = 0.4F;
			renderLight = light;
		}
		...
		// Drive the live-mob animation explicitly. NULL entity arg is contract-safe — all three
		// models' setAngles read only animationProgress/limbDistance (see class javadoc).
