...
public class Megalodon extends HostileEntity {
	 ...
	 * + deep-ocean-only biome attachment in OceanOverhaulWorldgen (≈ one prowling a
	...
	private void updateSegments(ServerWorld world) {
		...
		if (respawn) {
			...
			for (int i = 0; i < SEGMENT_OFFSETS.length; i++) {
				MegalodonSegment seg = new MegalodonSegment(OceanOverhaul.MEGALODON_SEGMENT, world);
				seg.setOwner(this);
